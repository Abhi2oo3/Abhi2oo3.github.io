...
const PeerRecommendations = ({ recommendations }: PeerRecommendationsProps) => {
  ...
  const renderRecommendation = (rec: Recommendation) => (
    <div className="flex items-start gap-4 mb-6 last:mb-0">
      <div className="w-16 h-16 rounded-full overflow-hidden flex-shrink-0 border-2 border-brand-cyan">
        <img src={rec.image} alt={rec.imageAlt} className="w-full h-full object-cover" />
      </div>
      <div className="flex-1">
        <h4 className="text-lg font-semibold text-text-primary">{rec.name}</h4>
        <p className="text-sm text-brand-cyan">{rec.role}</p>
        ...
        <div className="relative mt-3">
          <p className="text-text-secondary leading-relaxed pl-5 italic">
            "{rec.recommendation}"
          </p>
        </div>
      </div>
  ...
  return (
    <div className="bg-card border border-subtle rounded-xl p-6">
      ...
      {/* Desktop: Show both recommendations vertically */}
